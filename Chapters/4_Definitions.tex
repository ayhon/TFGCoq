...
The purpose of \mintinline{coq}{FORVES} is to certify that two sequences of EVM instructions are semantically
...
Therefore, we can define the state of an EVM application to be a function from \mintinline{coq}{nat} to \mintinline{coq}{EVMWord},
...
Since a program is just a function of type \mintinline{coq}{assignmet -> assignment} we can define semantic equality
...
\end{minted}
% TODO: Explain implicit arguments.
...
This is the general approach used in \mintinline{coq}{FORVES} to prove that two programs are equivalent. A set of
optimizations are defined under the \mintinline{coq}{optimizations/} folder. Each optimization defines a transformation
...
They appear to do the same, but actually the second snippet modifies the state of \mintinline{coq}{z} while the first one
doesn't. This is not relevant if \mintinline{coq}{z} is a local variable or if it's no longer used in the rest of the code,
...
optimization \mintinline{coq}{add_zero}, which says that if we have \mintinline{coq}{ADD x0 x1} and \mintinline{coq}{x0} is zero then this is equivalent to
just having \mintinline{coq}{x1}. This optimization can only be applied if we have the condition \mintinline{coq}{x0 = 0}, however this 
...
When we reach the \mintinline{coq}{else} branch what we have in our context is that $x \le 0$ and $x \ge 0$, not $x = 0$.
However, we know that having $x \le 0$ and $x \ge 0$ implies having the condition we need. Therefore, to 
apply the optimization \mintinline{coq}{add_zero} we need to be able to show that the current contextual information we
...
We call \mintinline{coq}{cliteral} to the elements we compare. In general, we're interested in comparisons between
variables, constants and variables offset by a constant. These are what our \mintinline{coq}{cliteral}s represent.
...
multiple nested conditions, these can be combined logically by using an \mintinline{coq}{and} connective. It could also 
...
we can combine the contextual information through an \mintinline{coq}{or} connective. 
...
The type \mintinline{coq}{constraints} represents the context that's available at one point in the code. What we want to
...
In logic, implication is usually expressed by the symbol $\Rightarrow$, although the symbol $\subset$ is
...
For any state, a condition \mintinline{coq}{c} may or may not hold. We say that a model satisfies a single constraint if
...
We can finally define what we mean by an implication checker. In the end, an implication checker is just
...
\mintinline{coq}{constaints -> constraint -> bool}, but in Coq we can go further and encode the implication checker's
...
That a \mintinline{coq}{imp_checker} can work as a \mintinline{coq}{conj_imp_checker} is obvious and not really useful, since a conjunction
is trivially in disjunctive normal form, where there are no disjunctions. The interesting side of this 
equivalence is the converse, showing that given a \mintinline{coq}{conj_imp_checker} we can create an \mintinline{coq}{imp_checker}. We do
this by defining the \mintinline{coq}{mk_imp_checker} function, of signature \mintinline{coq}{conj_imp_checker -> imp_checker}.
...
the \mintinline{coq}{Program} prefix to this definition, to reassure Coq that we know that the definition isn't finished 
...
Before we conclude this chapter, we must highlight an important fact. We've only
